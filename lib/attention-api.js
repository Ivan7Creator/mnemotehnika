...
async function requestOpenAI(apiKey, { input }) {
  ...
  if (!response.ok) {
    ...
    throw new OpenAIRequestError(payload?.error?.message || "OpenAI request failed.", {
...
function openAIErrorResponse(error) {
  if (error?.code === "invalid_api_key") {
    return jsonResponse(401, {
      error: "Неверный OpenAI API ключ. Обнови OPENAI_API_KEY в .env и перезапусти сервер.",
    });
  }

  if (error?.status === 429) {
    return jsonResponse(429, {
      error: "OpenAI временно отклонил запрос: лимит или баланс. Проверь аккаунт и попробуй позже.",
    });
  }

  return jsonResponse(502, {
    error: "Не удалось получить ответ от OpenAI. Проверь ключ, модель и подключение.",
  });
}
